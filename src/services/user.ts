 ...
 * @returns {Promise<string>} - Promesa que se resuelve con la descripción del rol.
 */
export const getRol = async (userRolID: S_USU["ROL_COD"]) => {
  // Realiza una petición POST a la API para obtener el rol asociado al usuario
  const { data } = await axios.post<S_ROL>(`${base_url}/api/users/rols`, { userRolID })
  // Devuelve la descripción del rol (ROL_DES)
  return data.ROL_DES
}
